...
# create several BashOperators and return those tasks in array 
...
# init dbt tasks as map
# one is for test the other is for run 
dbt_tasks = {}

for node in data["nodes"].keys():
    if node.split(".")[0] == "model":
        node_test = node.repalce("model", "test")
        dbt_tasks[node] = make_dbt_task(node, "run")
        dbt_tasks[node_test] = make_dbt_task(node, "test")

for node in data["nodes"].keys():
    if node.split(".")[0] == "model":
        # here we set dependency to run tests on a model after model runs finishes 
        node_test = node.replace("model", "test")
        dbt_tasks[node] >> dbt_tasks[node_test]

        # here we set all model -> model dependnecies 
