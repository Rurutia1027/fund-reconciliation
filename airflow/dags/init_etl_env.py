...
dag = DAG (
    ...
    start_date=datetime(2025, 1, 1, 0, 0, 0, 0, pytz.UTC),
    tags=['init'],
...
create_schema_task = PostgresOperator(
    ...
    Create schema if not exsits ops; 
...
create_xml_destination_task = PostgresOperator(
    ...
    sql = """
    CREATE TABLE IF NOT EXISTS imprt.ResellerXML (
        EntryID serial PRIMARY KEY,
        Data jsonb, 
        Loaded_Timestamp timestamp not null default now()
    ); 
...
create_channels_destination_task = PostgresOperator(
    ...
    sql = """
    CREATE TABLE IF NOT EXISTS imprt.channels(channel_id int, 
...
create_customers_destination_task = PostgresOperator(
    ...
    sql = """
    CREATE TABLE IF NOT EXISTS import.customers(customer_id int, first_name VARCHAR(255), 
        email VARCHAR(255),
...
create_channels_task >> create_registry_table >> [create_csv_destination, create_xml_destination_task]
create_schemas_task >> [create_transaction_task, create_resellers_destination_task, create_channels_destination_task, create_customers_destination_task, create_products_destination_task]